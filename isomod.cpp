...
void printhelp() {
	printf("Commands:\n  replace [isofile] [targetfile] [hostfile]\n");
}
...
int main(int argc, char *args[]) {
	...
	if (argc <= 1) {
		...
		char finaliso[strlen(isofilename) + 3];
		sprintf(finaliso, "./%s", isofilename);
		FILE *isofile = fopen(isofilename, "r+");
		if (NULL == isofile) {
			fprintf(stderr, "Couldn't open ISO %s.\nCheck if anything else has it open.\n", isofilename);
			return 1;
		}
		printf("%lx\n", ftell(isofile));
